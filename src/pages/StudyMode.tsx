...
export const StudyMode = () => {
  ...
  const selectedCurriculum = useMemo(
    () => curricula.find((item) => item.id === curriculum),
  ...
  async function startStudy() {
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          prompt: `Create ${questions} ${effectiveType} questions about "${topic}" in ${subject} for ${classLevel}, ${department}, following ${curriculumName}.
...
{!isExternalCurriculum ? (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="space-y-2">
          ...
          <span className="text-xs text-white/40">
            {studyMode === 'exam'
              ? 'Exam Mode uses objectives only and reveals results at the end.'
              : 'Test Mode uses objectives for immediate evaluation. Theory and mixed practice are also available when enabled.'}
          </span>
        </label>
      </div>
